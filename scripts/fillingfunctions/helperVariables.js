import { rankedHandRankings } from './handRanking.js'
...
const suitsShort = suits.map(suit => suit.charAt(0).toLowerCase())
...
export const cardsCombosTable = buildAllCombos(cardsTable)


function buildAllCombos(cardsTable) {
  cardsTable.forEach(row => {
    row.forEach(combo => {
